...
OrderManagementSystem::OrderManagementSystem(ExchangeConnector& a_connector, EventLog* a_log,
                                             RiskConfig a_risk)
    : connector_(a_connector), log_(a_log), risk_(std::move(a_risk))
{}
...
auto OrderManagementSystem::projected_position(const std::string& a_symbol,
                                               ...
                                               const std::string& a_candidate_qty)
    -> std::string
{
    Decimal projected{};
    ...
    for (const auto& [id, record] : orders_) {
        ...
        const auto filled = parse_decimal(record.filled_quantity.empty()
                                              ? "0"
        ...
        if (quantity.is_ok()) {
            add_in(signed_amount(record.side,
                                 sub_clamped_zero(quantity.value(), filled.value())));
        }
    }
    ...
    if (candidate_qty.is_ok()) {
        const auto candidate_filled =
            parse_decimal(a_replace != nullptr && !a_replace->filled_quantity.empty()
                              ? a_replace->filled_quantity
                              : "0");
        if (candidate_filled.is_ok()) {
            add_in(signed_amount(
                a_candidate_side, sub_clamped_zero(candidate_qty.value(),
...
auto OrderManagementSystem::place(const OrderRequest& a_request) -> Result<PlaceOutcome>
{
    const std::lock_guard lock(mutex_);
    ...
    if (limits.has_value()) {
        const RiskOrder risk_order{.side = a_request.side,
                                   ...
                                   .projected_position = projected_position(
                                       a_request.instrument_id, nullptr, a_request.side,
                                       a_request.quantity)};
    ...
    // ---- venue routing ----
    const auto placement = connector_.place_order(a_request);
    if (!placement.is_ok()) {
...
auto OrderManagementSystem::cancel(std::string_view a_client_order_id) -> Result<OrderRecord>
{
    ...
    if (is_terminal(record.state)) {
        return Error{"order_terminal",
                     "order " + record.client_order_id + " is " +
                         std::string{state_to_string(record.state)}};
    }

    const auto placement =
        connector_.cancel_order(CancelRequest{record.client_order_id, record.symbol});
    if (!placement.is_ok()) {
        return placement.error();
    }
...
auto OrderManagementSystem::amend(const AmendCommand& a_command) -> Result<OrderRecord>
{
    ...
    const auto placement = connector_.amend_order(
        AmendRequest{record.client_order_id, record.symbol, new_price, new_quantity});
    if (!placement.is_ok()) {
        return placement.error();
    }
                                              ...
                                              std::string_view a_quantity) -> bool
{
    ...
    if (!a_filled.empty()) {
        ...
        const auto current = parse_decimal(a_record.filled_quantity.empty()
                                               ? "0"
                                               : a_record.filled_quantity);
        if (reported.is_ok() && current.is_ok() &&
            compare(reported.value(), current.value()) > 0) {
...
void OrderManagementSystem::record_from_snapshot(const OrderSnapshot& a_snapshot, bool a_adopted)
{
...
auto OrderManagementSystem::reconcile() -> ReconcileReport
{
    ...
    // ---- Phase A: adopt venue-live orders missing from the registry ----
    const auto pending = connector_.get_open_orders();
    if (!pending.is_ok()) {
        ...
        for (const auto& snapshot : pending.value()) {
            ...
            if (it == orders_.end()) {
                record_from_snapshot(snapshot, true);
                ++report.adopted;
    ...
    for (const auto& [id, record] : orders_) {
        if (!is_terminal(record.state)) {
            open.emplace_back(id, record.symbol);
        }
    }
    for (const auto& [id, symbol] : open) {
        OrderRecord& record = orders_.at(id);
        const auto snapshot = connector_.get_order(OrderQuery{id, symbol});
        if (snapshot.is_ok() && snapshot.value().has_value()) {
            if (apply_observation(record, snapshot.value()->state,
                                  snapshot.value()->filled_quantity,
                                  snapshot.value()->average_fill_price,
                                  snapshot.value()->price, snapshot.value()->quantity)) {
                append_event(state_event(record));
                if (is_terminal(record.state)) {
        ...
        const bool absent =
            snapshot.is_ok() ||
            (snapshot.error().code == "venue:51603" || snapshot.error().code == "venue:51016");
        if (absent) {
            ...
            if (apply_transition(record.state, OrderState::Rejected) ==
                ...
                record.rejection = Error{"venue_absent",
                                         "order is unknown to the venue after reconciliation"};
                append_event(state_event(record));
...
auto OrderManagementSystem::load_from_log() -> Result<EventLog::ReplayStats>
{
    ...
    const std::lock_guard lock(mutex_);
    return EventLog::replay(log_->path(), [this](const nlohmann::json& a_event) {
        apply_log_event(a_event);
    });
}
...
void OrderManagementSystem::apply_log_event(const nlohmann::json& a_event)
{
    ...
    if (*type == "place_accepted" || *type == "adopted") {
        ...
        const auto order_type =
            parse_type(event_string(a_event, "orderType").value_or("limit"))
        ...
        if (*type == "adopted") {
            ...
            record.filled_quantity =
                event_string(a_event, "filledQuantity").value_or("0");
