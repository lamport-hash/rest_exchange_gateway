...
/// Order records carry "rejection": {"code", "reason"} when their state
/// is "rejected". a_oms must outlive the app. All error responses use
/// the structured schema {"error": {"code", "reason", "clientOrderId"}};
/// exceptions raised inside handlers are converted to structured 500
/// responses and never cross the REST boundary.
